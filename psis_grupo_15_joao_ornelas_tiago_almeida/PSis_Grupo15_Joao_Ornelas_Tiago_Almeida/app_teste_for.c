...
int main(int argc, char const *argv[]) {
    ...
    while (close!=1) {
        ...
        if(cmp==0){                                                    //If user action is copy
            ...
            while (tries<0||tries>100){
                printf("Enter number of copy's : \n");
                scanf("%d", &tries);
                ...
                for(int i=0; i<=tries;++i){
                    if(clipboard_copy(fd,region, (void*)buf, sizeof(buf))==0){   //Calls clipboardcopy with region selected by user and message casted as void*
